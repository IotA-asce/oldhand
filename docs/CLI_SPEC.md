...
`python src/oldhand/cli.py` commands rather than assuming a public package.
...
```bash
python src/oldhand/cli.py --version
python src/oldhand/cli.py init <path> [--json]
python src/oldhand/cli.py setup <claude|codex|cursor|opencode> [--apply|--undo]
python src/oldhand/cli.py validate [--json]
python src/oldhand/cli.py rebuild [--strict]
python src/oldhand/cli.py search "query"
python src/oldhand/cli.py search --history "query"
python src/oldhand/cli.py search "query" --collection <name> --scope <scope> --type <type> --topic <topic> --status <status> --importance <level> --limit N [--json]
python src/oldhand/cli.py show <id> [--json]
python src/oldhand/cli.py backlinks <id> [--json]
python src/oldhand/cli.py list [--history] [--type <type>] [--topic <topic>] [--status <status>] [--importance <level>] [--collection <name>] [--limit N] [--json]
python src/oldhand/cli.py topics [--collection <name>] [--limit N] [--json]
python src/oldhand/cli.py collections [--json]
python src/oldhand/cli.py stats
python src/oldhand/cli.py selftest
python src/oldhand/cli.py usage
python src/oldhand/cli.py new --title "..." --type <type> --importance <level> [...]
python src/oldhand/cli.py relate <source-id> <relation-type> <target-id>
python src/oldhand/cli.py unrelate <source-id> <relation-type> <target-id>
python src/oldhand/cli.py supersede <old-id> --by <replacement-id>
python src/oldhand/cli.py status <id> <current|resolved|deprecated|historical>
python src/oldhand/cli.py rename <old-id> <new-id>
python src/oldhand/cli.py topic <id> (--add <topic> | --remove <topic>)
python src/oldhand/cli.py classify <id> [--importance ...] [--scope ...] [--risk ...] [--durability ...] [--evidence ...]
python src/oldhand/cli.py compact --into <target-id> <source-id>... [--dry-run] [--json]
python src/oldhand/cli.py run-start --task <text> --evaluator <name> [--id <id>] [--policy <name>] [--goal maximize|minimize] [--workers N] [--workspace-ref <ref>] [--json]
python src/oldhand/cli.py attempt-add <run-id> --id <attempt-id> --parent <root|attempt-id> --proposal <text> [--artifact-ref <ref>] [--policy-version <name>] [--json]
python src/oldhand/cli.py attempt-evaluate <run-id> <attempt-id> --score N (--correct|--incorrect) --outcome <success|failure|error> [--cost N] [--duration-ms N] [--diagnostics-ref <ref>] [--json]
python src/oldhand/cli.py run-finish <run-id> [--json]
python src/oldhand/cli.py run-validate [<run-id>] [--json]
python src/oldhand/cli.py runs [--status active|completed] [--json]
python src/oldhand/cli.py run-show <run-id> [--json]
python src/oldhand/cli.py replay <run-id> --policy <breadth|depth|score-greedy> --budget N [--workers N] [--beta-cost N] [--beta-parallel N] [--json]
python src/oldhand/cli.py policy-compare <policy>... --budget N [--incumbent <policy>] [--holdout N] [--evaluator <name>] [--workers N] [--beta-cost N] [--beta-parallel N] [--json]
python src/oldhand/cli.py explore-context <query> --workers N [--history-branches N] [--per-branch N] [--json]
python src/oldhand/cli.py run-distill <run-id> <attempt-id> --title <text> --type <type> --importance <level> --topics <csv> [--id <id>] [--dry-run] [--json]
python src/oldhand/cli.py doctor [--limit N]
python src/oldhand/cli.py conflicts
python src/oldhand/cli.py eval [--save <name>] [--against <name>]
python src/oldhand/cli.py metrics [--full] [--export <path>]
python src/oldhand/cli.py obsidian
```

Each has its own section below. `oldhand --help` is authoritative if this list
and the parser ever disagree.

## Project-local harness setup

`oldhand setup` writes no files by default. It prints the exact unified diff for
a small, project-local instruction that tells an agent to search Oldhand before
changing an area with uncertain constraints. Pass `--apply` only after
reviewing that diff. `--undo --apply` removes only Oldhand's marked content.

The command never creates hooks, MCP configuration, user-level configuration,
credentials, or automatic command execution.

- `claude` creates the dedicated `.claude/rules/oldhand.md` file when it does not
  already exist.
- `cursor` creates `.cursor/rules/oldhand.mdc` as an agent-requested rule, not an
  always-applied rule.
- `codex` and `opencode` update only an already-existing root `AGENTS.md`;
  Oldhand will not create or overwrite that shared instruction file.
...
`oldhand run-start` creates a canonical discovery trace under `experience/runs/`.
Traces are deliberately separate from distilled `memory/` records and from the
disposable `.oldhand/` index. A run records its task, fixed evaluator, exploration
policy, score direction, worker budget, and optional workspace reference.

`oldhand attempt-add` appends an immutable proposal node. Root may open several
...
`oldhand attempt-evaluate` attaches the fixed evaluator's grounded result exactly
...
`oldhand run-finish` closes only a nonempty run whose attempts all have grounded
...
`oldhand runs` is the deterministic discovery catalog, including attempt,
...
`oldhand replay` evaluates a deterministic exploration policy against a completed
...
`oldhand policy-compare` evaluates the incumbent and every candidate on the same
completed histories. The newest `--holdout N` traces are reported separately;
Oldhand never rewrites or promotes policy code automatically. Histories from
different evaluators must be selected explicitly rather than averaged.

`oldhand explore-context` prevents one historical direction from collapsing every
...
`oldhand run-distill` closes the experience-to-memory loop. It accepts only an
...
`oldhand init PATH` creates a new archive with `memory/README.md` and an empty
...
`oldhand validate --json` returns record, error, and warning counts plus the full
...
1. `--root PATH`;
2. the `OLDHAND_ROOT` environment variable (set by `tools/install.py`, see
   `INSTALL.md`);
...
One database at `<root>/.oldhand/oldhand.db` holds all collections. Record `id`
...
Pass `--json` when another tool or agent will consume the results. Oldhand emits
...
`oldhand list` browses records deterministically by title, then id, without
...
`oldhand topics` discovers the vocabulary needed by exact topic filters. It lists
...
`oldhand topic ID --add TOPIC` and `--remove TOPIC` safely curate a canonical
record's vocabulary. Matching is case-insensitive and space/hyphen aware;
Oldhand refuses to remove the final topic.

`oldhand classify ID` updates one or more ranking and governance fields in a
single validated write. Enum choices are enforced by the CLI parser.

`oldhand compact --into TARGET SOURCE...` is deliberately conservative: prepare
the target's canonical prose first, then Oldhand atomically marks every source
...
`oldhand collections` lists every indexed collection with its kind, active and
...
`oldhand selftest` asserts these invariants. Run it after changing any scoring
...
`oldhand rename OLD NEW` changes a canonical record id and every incoming
...
`rebuild` recreates `.oldhand/oldhand.db`, populates FTS5, and regenerates each
...
Every `search` and `show` appends one line to `.oldhand/retrieval.jsonl`:
...
`oldhand doctor` runs health checks that need no hand-written ground truth. The
...
`oldhand conflicts` finds records that disagree with each other or with
...
`oldhand obsidian` generates navigation notes for viewing the archive as an
...
`memory/`. Oldhand only scans `*/memory/**`, so generated notes are invisible to
the index and can never fail validation. Re-run it after any rebuild.

## Eval

`oldhand eval` measures retrieval against `eval/queries.jsonl`, one JSON object
per line: `{"q": "...", "expect": ["<id>", ...]}`.

```bash
oldhand eval --save baseline       # record where things stand
oldhand eval --against baseline    # after a change: what moved, and which way
```
...
`oldhand usage` reads `.oldhand/retrieval.jsonl` and answers the question that
...
`oldhand new` writes a well-formed skeleton and generates the bookkeeping the
...
without `--id`, Oldhand keeps generating a unique id from the title.

## Relationship maintenance

`oldhand relate SOURCE TYPE TARGET` adds one typed relationship to the canonical
...
unless the target is already retired. Oldhand refuses to mutate an archive that
...
`oldhand unrelate SOURCE TYPE TARGET` removes one existing relationship and
...
`oldhand supersede OLD --by NEW` performs the two sides of supersession together:
...
`oldhand status ID STATUS` changes ordinary lifecycle states with the same safe
